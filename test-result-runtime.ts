...
import { createSandboxedBashOps } from "./src/sandbox-runtime.ts";

const cwd = "/home/hermes/omp-sandbox-dev/test-project";
const secret = "/home/hermes/omp-sandbox-dev/test-home/carderne-runtime-secret.txt";
await mkdir(cwd, { recursive: true });
await writeFile(secret, "CARDERNE_HOST_SECRET\n");
const hostPid = String(process.pid);
const config = {
  network: { allowedDomains: [], deniedDomains: [], sshProxy: false },
  filesystem: {
    denyRead: ["/home"],
    allowRead: [cwd, "/home/hermes/omp-sandbox-dev/plugins/pi-sandbox-carderne/node_modules/@carderne/sandbox-runtime/vendor"],
    allowWrite: [cwd],
    denyWrite: [],
  },
...
const ops = createSandboxedBashOps("/bin/bash", false);
const probes = [
  ["write-project", `printf CARDERNE_OK > ${cwd}/carderne-inside.txt`, "pass"],
  ["write-outside", `printf ESCAPE > /home/hermes/omp-sandbox-dev/test-result/carderne-outside.txt`, "deny"],
  ["read-host-secret", `test "$(cat ${secret})" = CARDERNE_HOST_SECRET`, "deny"],
  ...
  ["nested-eval", `eval 'printf NESTED_OK > ${cwd}/carderne-nested.txt'`, "pass"],
] as const;
const results = [];
